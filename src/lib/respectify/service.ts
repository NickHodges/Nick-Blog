...
import { RespectifyClient, type CommentScore, AuthenticationError, PaymentRequiredError, RespectifyError } from '@respectify/client';
  ...
  try {
    ...
    if (result.comment_score) {
      ...
      if (result.comment_score.logical_fallacies.length > 0) {
        suggestions.push(`Logical fallacies detected: ${result.comment_score.logical_fallacies.map((f) => f.fallacy_name).join(', ')}`);
      }
